...
> 路由的基础机制（请求分发、负载均衡、健康检查、Kubernetes 服务发现）参见 [vLLM Router 概述](router.md)。本文聚焦于路由的**决策智能**——信号驱动的语义路由如何决定请求去向。

**vLLM Semantic Router**（以下简称 VSR）正是为解决这一问题而诞生的开源框架。它的论文 _"When to Reason: Semantic Router for vLLM"_ [6] 被 NeurIPS 2025 MLForSys Workshop 接收，提出了一种**信号驱动的决策路由**架构，在 MMLU-Pro 基准上实现了 **10.2% 的准确率提升**，同时**降低了 47.1% 的延迟**和 **48.5% 的 Token 消耗**。
...
语义路由的必要性源于一个事实：模型生态已经碎片化到单模型无法覆盖所有请求。本节先刻画碎片化的形态，再从信息论视角剖析路由问题的本质——最终收敛到路由决策本身的三个内在矛盾，为后文每一章提供要解决的问题。
...
**第一层**是 Shannon 1948 年提出的概率信息论 [4]——通过信号提取最大化与路由结果的互信息；**第二层**是 Shannon 1938 年硕士论文的开关电路代数 [5]——通过 $\{\text{AND}, \text{OR}, \text{NOT}\}$ 组合信号条件来表达任意路由策略。信号向量 $S(r)$ 正是这两个域的接口。
...
路由决策本身为什么难？把它拆开看，有三个**内在矛盾**——不是实现得不够好，而是任何路由系统都必须正面回答的问题。全文的架构设计，本质上就是对这三个矛盾的应答：

**矛盾一：延迟与精度的矛盾。** 路由决策发生在请求处理的最前端，它每花 1ms，TTFT 就多 1ms——但语义理解（领域、意图、复杂度）恰恰需要 ML 模型，而 ML 推理的延迟在 10-120ms 量级。路由要么选快（关键词、正则，但理解浅），要么选准（嵌入、分类器，但拖慢响应）。能否同时要快和准？

**矛盾二：可解释与表达力的矛盾。** 路由策略在合规场景（医疗、金融）必须可审计——"为什么这个请求去了这个模型"要有明确答案；但复杂的生产策略（多条件组合、跨提供商故障转移、多轮上下文）又要求表达能力。布尔规则可解释但呆板，神经网络表达力强但不可解释。能否用可解释的语法表达任意策略？

**矛盾三：组合与统一的矛盾。** 不同的部署场景（医疗合规、开发者工具、多云企业）需要完全不同的路由行为——但底层是同一套引擎。如果每个场景都写一套特殊逻辑，系统会分裂成互不相通的碎片；如果强行统一，每个场景都得不到满足。能否用"配置"而非"代码"切换场景，同时保持统一的行为语义？

后文每一章的展开都对应其中一个矛盾的解法：第 2 章的三层架构回答组合-统一，第 3 章按需计算 + 并行评估回答延迟-精度，第 4 章的布尔完备性回答可解释-表达力。第 12 章再回头评估这些解法各自的代价。

### 1.4 真实生产路由的完整需求

除了三个内在矛盾，生产路由系统还必须覆盖一组正交的功能需求——这些需求在论文中被列为设计目标，也是后文各章逐一落地的对象：
...
第 1 章把路由拆成了三个内在矛盾——其中"组合-统一"矛盾（不同场景需要不同行为，但底层要同一套引擎）是架构层的总纲。VSR 对它的解法是**可组合性**：将复杂的路由逻辑分解为三个正交的层，每层通过明确定义的接口协作，使得不同部署场景通过配置而非代码来切换。
...
三层架构的可行性依赖四条原则，每一条都是对"配置驱动"这一承诺的兑现：
...
三层架构确定了信号在系统中的位置——它是决策层的输入。但第 1 章的**延迟-精度矛盾**在这里立刻成为现实：语义理解需要 ML 模型（10-120ms），而路由发生在一线请求路径上。VSR 的解法是把信号拆成两级——毫秒级启发式 + 学习型——再配以"按需计算 + 并行评估"把墙钟时间压到可接受范围。本节详细介绍 13 种信号类型及其实现机制。
...
| **Preference** | 基于用户交互历史的个性化路由（默认调用外部 LLM，`Generate` 至 vLLM 端点，MaxTokens=1000） | 200–500ms |

**关键优化——按需计算**：引擎只计算被至少一个活跃决策引用的信号类型。典型配置中只有 3-5 个信号类型活跃，这比穷举计算减少了 50-70% 的提取延迟。所有信号并行评估，因此墙钟时间取决于最慢的活跃信号而非总和——典型配置（不含 Preference 这类 LLM 型信号）下约 120ms（对应论文给出的 ML 信号 10-120ms 范围上限）。一旦启用 Preference，墙钟时间由外部 LLM 调用决定（200-500ms）。
...
信号层产出了一组结构化的信号结果，但"有信号"不等于"有决策"——把 13 种信号组合成一条路由规则，正是第 1 章**可解释-表达力矛盾**的战场。VSR 的选择是布尔代数：用 $\{\text{AND}, \text{OR}, \text{NOT}\}$ 构建任意嵌套的表达式树，既保证策略可审计（每个决策就是一条可读的布尔公式），又借助功能完备性定理证明表达力不设上限。本节介绍布尔表达式树的构建、功能完备性保证以及两种选择策略。
...
决策引擎回答了"请求该走哪条路由"，但每条路由背后往往挂着多个候选模型——"选哪个"是另一个独立问题。第 1 章提到的成本模型选择需求在这里具体化：VSR 集成了七大家族、十三种算法，覆盖从静态评分到强化学习的多种选择策略。这一章的读法不是"记下十三种算法"，而是理解**选择算法要优化的目标不同**——有的追求质量稳定（Rating-Based），有的追求延迟优先（Latency-Aware），有的用历史数据学习（Classical ML），有的干脆不选而是组合（ReMoM）。
...
ReMoM (Reasoning for Mixture of Models) 是 VSR 与其余 12 种算法结构上最不同的一个——它不是"选一个模型"，而是**多轮并行推理 + 合成**：每轮同时让多个模型回答问题，用合成 prompt 将响应融合后进入下一轮，逐轮收窄宽度，最终输出单一合成结果。它受 PaCoRe（arXiv:2601.05593）启发，实现位于 `pkg/looper/remom.go`，通过 `AlgorithmConfig.ReMoM` 配置（而非 `SelectionMethod` 枚举）启用。
...
Round 2 (b₂=2): Synthesized → [m_B, m_C] → 2 responses (并行)
                        ↓ Synthesis Prompt
Final (b=1):    Final synthesis → single output
```
...
`breadth_schedule`（代码定义：`ReMoMAlgorithmConfig.BreadthSchedule []int`，默认 `[4]`）的**每个元素对应一轮并行推理的宽度**——上述 `[4, 2]` 即第一轮并行 4 个候选、第二轮并行 2 个候选；最终合成轮固定输出单个结果，不计入 schedule。其他可调参数：`compaction_strategy`（轮间上下文压缩策略）、`max_concurrent`（并行请求上限）、`synthesis_template`（合成 prompt 模板）、`include_intermediate_responses`（是否保留中间轮响应）。ReMoM 设计受 PaCoRe（arXiv:2601.05593）启发。
...
前三章构建了"信号 → 决策 → 选择"的主链，但第 1 章的隐私安全需求暗示了一个架构选择：安全检测（越狱、PII）放在链路的哪个位置？串行插件（先安全检查、再路由）会叠加延迟；独立旁路又脱离路由上下文。VSR 的答案是**把安全检测做成信号**——与意图信号在同一层并行评估。这种架构实现了零额外延迟、可组合的安全策略和统一可观测性。
...
期望成本： $E[\text{Cost}] = C_{\text{sent}} + p_{\text{factual}} \cdot (C_{\text{det}} + \bar{k} \cdot C_{\text{nli}})$，其中 $C_{\text{sent}}$ 为门控分类器成本、$p_{\text{factual}}$ 为查询通过门控的概率、$C_{\text{det}}$ 为 span 检测器成本、$\bar{k}$ 为平均检测到的 span 数、$C_{\text{nli}}$ 为单条 NLI 解释成本
...
信号层依赖 9 种学习型信号，每一种背后都是 ML 推理——这带来一个此前各章回避的问题：**推理运行时放在哪**。如果路由进程内嵌 Python 推理栈，信号提取会背上 GIL 和解释器开销；如果外置成服务，每次信号提取都多一次网络往返。VSR 的工程答案是用 Rust 原生 ML 推理替代 Python，通过 C FFI 和 CGo 暴露给 Go 路由进程——消除了 Python 运行时开销，同时保持了高性能的 GPU 推理能力。这一选择不是性能洁癖：第 3 章的延迟-精度矛盾，最终是在运行时这一层才被彻底解决的。
...
前六章各自回答了系统的一个组件，但组件要成为系统，还需要回答：**这些机制以什么顺序、在什么时机作用于一个真实的请求？** VSR 的部署答案是作为 Envoy External Processor 运行——在 HTTP 请求的四个阶段被 Envoy 调用，通过透明拦截实现请求处理管道。本节详细介绍四个处理阶段和 10 步管道流程——它把信号提取、决策、缓存、选择、安全全部串成一条严格顺序的流水线。
...
请求/响应过滤器以 `req_filter_*.go` / `res_filter_*.go` 命名分布在 `pkg/extproc/` 中，分别对应信号提取、缓存、记忆、RAG、模态、图像生成、推理模式、系统提示等管道阶段。
...
第 8 章的管道把单次请求处理得井井有条，但第 1 章的多轮状态需求戳中一个盲区：**单次路由是无状态的，对话却是有状态的**——用户上一轮说"帮我改那段代码"，下一轮说"再改一下"，路由必须记得"那段代码"指的是什么。VSR 的记忆系统维护用户作用域的对话知识，支持情景记忆存储和混合检索。本节介绍 ReflectionGate 过滤机制和三路并行检索管道——它让路由决策在对话轮次间保持一致，而不只是逐请求独立判断。
...
- 每 $s$ 轮（存储间隔，默认 3）额外存储一个跨越最近 $w$ 轮（窗口宽度，默认 5）的滑动窗口块——捕捉跨轮次的上下文依赖
...
实现分布在 `pkg/memory/`（store/extractor/reflection/milvus_store），生产后端为 Milvus。
...
第 2 章承诺"不同场景通过配置而非代码切换"——但配置本身会膨胀：13 种信号、嵌套布尔规则、算法参数、插件链、后端端点，如果全部写进 YAML，配置文件会比代码更难维护。VSR 的回答是**为配置发明一种语言**：DSL 配置语言。VSR 将整个系统解释为一个可编程的神经符号推理引擎，DSL 正是这一设计理念的体现——信号是神经层，布尔规则是符号层，DSL 让两者在同一份文本中可组合。
...
本节展示 GPU 加速推理、LoRA 内存效率和端到端路由的实验数据（信号提取延迟已在 §3.1 给出）。
...
前十一章把 VSR 的设计讲成了一条顺理成章的解决链。但第 1 章承诺过：每个矛盾的解法都有代价。这一章把代价摆到桌面上——哪些是设计权衡（事实），哪些是推演（分析），分开标注。

### 12.1 路由决策本身的延迟成本

**事实**：信号提取的墙钟时间在典型配置下约 120ms，启用 Preference 后 200-500ms。**分析**：路由发生在请求路径的最前端——这 120ms 是叠加在 TTFT 上的。论文报告端到端延迟降低 47.1%（13.09s vs 24.76s），但那是模型推理占主导的长响应场景；对短响应、TTFT 敏感的场景（交互式聊天、代码补全），120ms 的路由决策可能吃掉响应预算的 10-30%。路由的收益来自"选对模型"——但选对的收益在长响应中才显著，而路由的成本在短响应中占比更高。**这是路由系统最根本的适用边界：收益与负载类型强相关，不是普适加速。**

### 12.2 ReMoM 的成本结构

**事实**：ReMoM 每轮并行 N 个模型调用，`[4, 2]` 配置即 4+2=6 次推理加 2 次合成。**分析**：token 消耗是单模型路由的 6 倍以上。它在质量敏感、延迟不敏感的场景（离线评测、复杂推理）成立；在成本敏感或高并发场景，等于把路由系统的成本放大一个量级。第 5 章的算法表里它被列为一种"选择算法"，但它和其余 12 种根本不在同一个成本维度——它是"用 N 倍成本买质量上限"，其余是"在固定成本内选最优"。

### 12.3 "安全即信号"的暗面

**事实**：安全信号与意图信号同池并行评估，墙钟取 max 而非 sum。**分析**：max 语义确实消除了"串行叠加"，但它没有消除竞争——墙钟时间取决于**所有活跃信号中谁最慢**。这意味着：在轻量配置（只有 keyword、language 等毫秒级信号）中启用安全信号，墙钟会从 <1ms 直接跳到安全信号的 55ms——安全**成为新的最慢信号**；而在已含 ML 信号的配置（最慢 60ms 的 domain）中，55ms 的安全信号藏在 max 之下，表面"零额外延迟"。所以"零额外延迟"只在"安全信号不是最慢"时成立——安全是新增计算，其成本从"串行加法"变成了"与意图信号竞争最慢位置"。轻量配置恰恰是安全最该便宜的地方，但 max 语义让它在轻量配置里最贵。

### 12.4 完备性 ≠ 易用性

**事实**：4.2 证明 AND/OR/NOT 可表达任意布尔函数。**分析**：理论完备性和实际可维护性之间隔着一条沟——表达一个"除 A、B、C 外的所有领域"的策略，YAML 需要写一层 NOT(OR(A,B,C))，而每个嵌套都增加心智负担。M≤50 决策、L≤10 条件的实践中，复杂策略的配置文件会膨胀到难以审查。完备性保证了"不存在表达不了的策略"，但没有保证"表达起来不难"。DSL（第 10 章）缓解了 YAML 的冗长，但把学习成本转移到了 DSL 语法本身。

### 12.5 十三种信号的维护成本

**事实**：每种学习型信号都是一个要训练、评估、监控的模型。**分析**：13 种信号 × 训练数据 + 精度监控 + 漂移处理，是持续的运维负担。第 3 章说"典型配置只激活 3-5 个"——但激活之外，未使用的信号仍然需要维护（代码、依赖、模型注册）。VSR 的"正交性"设计让新增信号只需实现一个评估函数，但生产环境中每一个信号都是长期负债。

### 12.6 故障面的不对称

**事实**：VSR 是 Envoy 的 ExtProc，位于请求关键路径。**分析**：路由到错误模型 = 一次质量损失（可接受）；路由进程本身故障 = 全部请求失败（灾难）。13 信号 × 13 算法 × 4 运行时 × DSL 编译器的复杂度意味着故障面远大于一个简单的静态路由。文章和论文都没有系统讨论**降级策略**——ExtProc 超时后 Envoy 是放行还是拒绝？路由进程崩溃时是否有 fallback 到直连后端？这是生产部署前必须回答的问题。

### 12.7 与轻量方案的分工

**事实**：RouteLLM [1] 训练一个分类器完成双模型路由，RouterDC [2] 用双对比嵌入。**分析**：VSR 的能力密度（13 信号、13 算法、安全、记忆、RAG、DSL）是这些轻量方案的 10 倍以上，但复杂度也随之而来。这不是缺点——它服务的是"异构模型集群 + 合规 + 多云"的重型场景。真正的问题在于：**什么时候该用 VSR，什么时候该用 RouteLLM？** 判断标准是需求维度：如果你只需要"难的问题走大模型，简单的问题走小模型"，一个二分类器就是最优解；当你需要可审计的策略、跨提供商的认证、多轮记忆时，才值得引入 VSR 的完整栈。

---

## 13. 总结与展望

本文从第 1 章的三个内在矛盾出发，沿着"架构 → 信号 → 决策 → 选择 → 安全 → 运行时 → 管道 → 记忆 → DSL"的解决链，完整解析了 vLLM Semantic Router；第 12 章又把每个解法的代价摆回桌面。现在可以做一个有边界的总结——它擅长什么、在什么边界内成立、往哪走。
