...
| $T(p)$ | $T(p) = L(p) \times (1 + \mathrm{queue depth}(d) \times \mathrm{avg wait})$ | 在线队列监控              |
...
$$\text{KV Size} = 2 \times n_{\text{layers}} \times n_{\text{heads}} \times d_{\text{head}} \times \mathrm{seq len} \times \mathrm{dtype size}$$
...
$$\alpha, \beta, \gamma, \lambda = f(\mathrm{SLA type}, \mathrm{runtime metrics}, \mathrm{historical performance})$$
...
$$\mathrm{model variant} = (\text{model}, \text{precision}, \text{backend})$$
