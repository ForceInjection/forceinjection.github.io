...
- **[`llm_memory_analysis.pptx`](llm_memory_analysis.pptx)**：配套讲稿（可用于内部培训或评审汇报）
...
- **[`qwen3_06b_config.json`](qwen3_06b_config.json)**：Qwen3-0.6B 的 HuggingFace `config.json` 样例，供脚本读取
...
> **为什么需要 V4 专用脚本**：通用 MHA/GQA/MLA 公式在 DeepSeek V4 上会**严重低估** KV Cache 占用，因为 c4a/c128a 层压缩比不同、Indexer Cache 仅在 c4a 层存在，且 FP8 KV + FP4 Indexer 的混合精度使简单乘 2（FP16）不再成立。详见 [`vllm/module_analysis/deepseek_v4_attention_support.md`](../vllm/module_analysis/deepseek_v4_attention_support.md)。
