...
- **[CXL 命令行工具链全景](pcie/09_cxl_cli_tools.md)**：cxl / daxctl / ndctl 三工具的分工与内核模块依赖、五族子命令与协议概念的映射、从插卡到可用内存的完整命令序列（create-region → enable-region → 形态转换）、无设备环境的输出画像与排查顺序、memtier 性能验证与工具链成熟度边界（含版本差异实测发现）。
